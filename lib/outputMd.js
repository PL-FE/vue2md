...
function outputMd (options) {

  const head = renderHead(options)
  const props = renderProps(options)
  const slots = renderSlots(options)
  const events = renderEvents(options)
  const methods = renderMethods(options)
  const data = renderData(options)
  const computed = renderComputed(options)
...
function renderHead ({ component, inputPath }) {
